...
urlpatterns = [
    ...
    path(
        "api/v1/user/<uuid:user_id>/",
        UserRetrieveUpdateDestroyAPIView.as_view(),
        name="user_detail",
    ),
    path(
        "api/v1/habits/<uuid:user_id>/",
        HabitListCreateAPIView.as_view(),
        name="user_habits",
    ),
    path(
    ...
    path(
        "api/v1/habits/<uuid:user_id>/dates/",
        HabitDatesListAPIView.as_view(),
        name="habit_dates",
    ),
    path(
        "api/v1/habits/<uuid:user_id>/<int:habit_id>/dates/",
        CurrentHabitDatesListAPIView.as_view(),
        name="current_habit_dates",
    ),
    ...
    path(
        "api/v1/avatargroup/", AvatarGroupListAPIView.as_view(), name="avatargroup_list"
    ),
    path("api/v1/icons/", IconListListAPIView.as_view(), name="icon_list"),
    path(
        "api/v1/progresspag/<uuid:user_id>/<int:habit_id>/",
        HabitProgressPagAPIView.as_view(),
        name="habit_progress",
    ),
    path(
        "api/v1/commonprogresspag/<uuid:user_id>/",
        CommonHabitProgressPagAPIView.as_view(),
        name="common_habit_progress",
    ),
    path(
        "api/v1/progress/<uuid:user_id>/<int:habit_id>/",
        HabitProgressAPIView.as_view(),
        name="habit_progress",
    ),
    path(
        "api/v1/commonprogress/<uuid:user_id>/",
        CommonHabitProgressAPIView.as_view(),
        name="common_habit_progress",
    ),
]
...
router.register(
    r"api/v1/habit-templates", HabitTemplateViewSet, basename="habit-template"
)
